...
\begin{defi}
(Degenerate pertubation theory) Notice that multiple states occupy the same energy level for the hydrogen atom with a Coulomb potential ($e^2/r$). Contrarily from the previous case where we saw one ket per energy level, we will try to see what happens when we deal with Hilbert spaces with multiple states per energy level. The state space with identical energy levels can be expressed as (via projection)
